...
def process_player_row(player_row, sport):
    ...
    for col in player_row:
        if CLASS['FULL_NAME'] in col['class']:
            ...
            if col.img:
                if col.img['src'].split('&'):
                  base_url = col.img['src'].split('&')[0]
                  if base_url.split('image_path='):
                    base_url = "%s?width=55&height=67" % base_url.split('image_path=')[-1]
                    player.setImage(base_url)
